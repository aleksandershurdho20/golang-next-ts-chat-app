...
// ReadMessage reads messages from the WebSocket connection and broadcasts them to the hub.
...
// WriteMessage writes messages from the client's send channel to the WebSocket connection.
func (c *Client) WriteMessage() {
	...
	for {
		// If the channel is closed (!ok), it means the client has been unregistered,
		// so it sends a WebSocket close message and returns, ending the goroutine.
